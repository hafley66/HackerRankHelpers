...
export var DFS = (function DFS(){
	...
	var Methods = {
		init() {
			this.G.V().forEach(v=>this._initVertex(v));
		},
		_initVertex(vertex){
			this.P(vertex, null);
			this.S(vertex, WHITE);
		...
		_Adj(vertex=this.curr.source) {
			return this.Adj(vertex) || vertex.E();
		},
